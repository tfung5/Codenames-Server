const { RED } = require("../constants/Cards");

const presetBoard = [
  [
    { word: "Leprechaun", color: "RED", state: "UNCHOSEN", row: 0, col: 0 },
    { word: "Sniper", color: "RED", state: "UNCHOSEN", row: 0, col: 1 },
    { word: "Volcano", color: "RED", state: "UNCHOSEN", row: 0, col: 2 },
    { word: "Undertaker", color: "GRAY", state: "UNCHOSEN", row: 0, col: 3 },
    { word: "Antarctica", color: "BLUE", state: "UNCHOSEN", row: 0, col: 4 },
  ],
  [
    { word: "Pumpkin", color: "BLUE", state: "UNCHOSEN", row: 1, col: 0 },
    { word: "Telescope", color: "GRAY", state: "UNCHOSEN", row: 1, col: 1 },
    {
      word: "Alien",
      color: "BLACK",
      state: "UNCHOSEN",
      row: 1,
      col: 2,
    },
    { word: "Greece", color: "RED", state: "UNCHOSEN", row: 1, col: 3 },
    { word: "Box", color: "RED", state: "UNCHOSEN", row: 1, col: 4 },
  ],
  [
    { word: "Dessert", color: "BLUE", state: "UNCHOSEN", row: 2, col: 0 },
    { word: "Winter", color: "BLUE", state: "UNCHOSEN", row: 2, col: 1 },
    { word: "Sky", color: "GRAY", state: "UNCHOSEN", row: 2, col: 2 },
    {
      word: "Apple",
      color: "BLUE",
      state: "UNCHOSEN",
      row: 2,
      col: 3,
    },
    {
      word: "Eye",
      color: "RED",
      state: "UNCHOSEN",
      row: 2,
      col: 4,
    },
  ],
  [
    { word: "Fork", color: "RED", state: "UNCHOSEN", row: 3, col: 0 },
    { word: "Motorcycle", color: "GRAY", state: "UNCHOSEN", row: 3, col: 1 },
    { word: "Dinosaur", color: "BLUE", state: "UNCHOSEN", row: 3, col: 2 },
    { word: "Nurse", color: "GRAY", state: "UNCHOSEN", row: 3, col: 3 },
    {
      word: "Doctor",
      color: "RED",
      state: "UNCHOSEN",
      row: 3,
      col: 4,
    },
  ],
  [
    { word: "Plate", color: "GRAY", state: "UNCHOSEN", row: 4, col: 0 },
    { word: "Space", color: "BLUE", state: "UNCHOSEN", row: 4, col: 1 },
    { word: "Happy", color: "RED", state: "UNCHOSEN", row: 4, col: 2 },
    { word: "Jester", color: "GRAY", state: "UNCHOSEN", row: 4, col: 3 },
    { word: "Riddle", color: "BLUE", state: "UNCHOSEN", row: 4, col: 4 },
  ],
];

const cardCounters = {
  redCardCounter: 9,
  blueCardCounter: 8,
  blackCardCounter: 1,
};

const startingTeam = RED;

module.exports = {
  presetBoard,
  cardCounters,
  startingTeam,
};
